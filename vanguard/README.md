> **This folder is a deployed copy. Do not edit anything here by hand.**
>
> The game's source lives in its own repository:
> https://github.com/abdulalrubat-bit/vanguard-orbit-tactical-strike
>
> This copy exists so the game is playable at
> `abdulalrubat-bit.github.io/vanguard/` — the same arrangement Rivenmark
> uses. An edit made here would be overwritten by the next sync and, worse,
> would not change `sw.js`'s cache key, so nobody with the game installed
> would ever see it.
